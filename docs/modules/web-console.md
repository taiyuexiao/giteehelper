...
- `/audit`：资源/动作筛选；
- `/integrations`：Gitee/飞书连接、Secret Store 配置、项目配置、运行时状态和检查清单。
...
- 桌面侧栏、平板双列、移动单列；
- 不把完整代码发送到飞书；
- 页面只显示配置状态，不显示 Token/Secret；Secret 以留空保持不变的方式替换。
...
| 2026-09-26 | 重排接入设置布局，增加 Web 配置、Secret Store 状态和运行时状态 | 接入配置 |
